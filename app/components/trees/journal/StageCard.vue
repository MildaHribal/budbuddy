  ...
  >
    ...
    >
      <div class="relative w-full h-48 overflow-hidden bg-black/20">
        <img :src="image" :alt="title" class="w-full h-full object-contain p-4" />
      </div>

      <div class="p-4">
        <h4 class="text-white font-semibold text-lg mb-1" :class="{ 'text-[color:var(--accent)]': isActive }">{{ title }}</h4>
      ...
      >
        <div v-if="isActive" class="absolute inset-0 bg-black/60 flex items-center justify-center z-10 backdrop-blur-[1px]">
          <button
